...
export const VoiceCard = ({ repo, onSelect }) => {
  return (
    <div className="voice-card" onClick={onSelect}>
      <div className="voice-avatar-container">
        <div className="voice-avatar">
          <img
            src={`https://api.dicebear.com/9.x/glass/svg?seed=${repo.name}`}
            alt={repo.name}
      ...
      <div className="voice-info">
        <div className="voice-header">
          <span className="voice-name">{repo.name}</span>
          <span className="github-owner">{repo.owner}</span>
        </div>
        <div className="voice-desc">{repo.desc}</div>
      </div>
    </div>
  );
};
